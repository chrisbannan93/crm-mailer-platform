...
import { loadPublicSiteContent } from './services/publicSiteLoader.js';
...
async function main() {
  ...
  const publicSiteContent = await loadPublicSiteContent(config.vertical);
  ...
  const app = createServer(config, service, publicSiteContent);
